...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> forgetRequestPassword(String username, String passwordNew, String forgetToken) {
        ...
        String tonKen = RedisShardedPoolUtil.get(Const.TOKEN_PREFIX+username);

        if (StringUtils.isBlank(tonKen)) {
            return ServerResponse.createByErrorMessage("token无效或过期");
        }
        if (StringUtils.equals(forgetToken, tonKen)) {
    ...
    public ServerResponse<User> updateInfomation(User user) {
        // username 不能被更新 email 也要进行一个校验, 校验新的email是不是已经存在, 并且存在的email如果相同的话, 不能是我们当前用户的
